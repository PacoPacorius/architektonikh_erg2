...
Όμοια με specbzip:
* Specmcf `3GHz/1GHz -> 2.83`, `3GHz/2GHz -> 1.46`, `2GHz/1GHz -> 1.94`. Οι τιμές είναι πολύ πιο κοντά στο να κάνουν τέλεια scale από specbzip, αλλά ακόμα δεν είναι τέλεια η αναλογία  

* Spechmmer `3GHz/1GHz -> 2.987`, `3GHz/2GHz -> 1.497 , 2GHz/1GHz -> 1.995`. Σε αυτό το benchmark μπορούμε να πούμε ότι ο χρόνος εκτέλεσης κάνει τέλεια scale με το cpu clock  
* Specsjeng `3GHz/1GHz -> 2.38`, `3GHz/2GHz -> 1.33`, `2GHz/1GHz -> 1.78`. Οι τιμές έχουν αρχίσει να αποκλίνουν, δεν κάνει scale τόσο καλά αυτό το benchmark.  
* Speclibm `3GHz/1GHz -> 1.82`, `3GHz/2GHz -> 1.19`, `2GHz/1GHz -> 1.52`. Είναι προφανές ότι οι τιμές δεν κάνουν scale εδώ.  
...
Παρατηρώντας τα αποτελέσματα βλέπουμε ότι σα περισσότερα benchmarks οι βελτιώσεις λόγω της αύξησης του μεγέθους του cache line είναι σημαντικές μέχρι το 128 στα περισσότερα benchmarks, άρα διαλέγουμε αυτό σαν τιμή μεγέθους cache line. 
...
![charts/specmcf_best_run.png](charts/specmcf_best_run.png)
![charts/specsjeng_best_run.png](charts/specsjeng_best_run.png)
![charts/speclibm_best_run.png](charts/speclibm_best_run.png)


|                                   | specbzip | spechmmer | specmcf  | specsjeng | speclibm |
| --------------------------------- | -------- | --------- | -------  | --------- | -------- |
| L1i assoc = 2 && L2 size = 32kB   | 2.873898 | 1.345467  | 1.502910 | 4.047122  | 2.318032 |
| L1i assoc = 2 && L2 size = 256kB  | 2.099995 | 1.285993  | 1.437419 | 3.798394  | 2.421434 |
| L1i assoc = 4 && L2 size = 32kB   | 2.873275 | 1.345312  | 1.408589 | 3.998215  | 2.318032 |
...
από την L2, άρα κόστος L1/κόστος L2 = A = 4/1. Το μέγιστο μέγεθος L2 cache που μπορούμε να χρησιμοποιήσουμε είναι 4MB και το μέγιστο μέγεθος L1 cache συνολικά είναι 256kB, άρα θα 
κανονικοποιήσουμε τη συνάρτησή μας με αυτές τις τιμές. Βλέπουμε ότι από τις σημειώσεις cache basics, ότι το κόστος του associativity μιας cache αυξάνει όσο αυξάνει και το μέγεθος της 
cache λόγω των επιπρόσθετων multiplexers και συγκριτών. Όσο ευξάνεται το cache line size θα έχουμε παραπάνω κόστος λόγω περισσότερων καλωδίων/pins που πρέπει να υλοποιήσουμε, καθώς 
χρειαζόμαστε παρπάνω χώρο. Άρα η συνάρτηση κόστους μας διαμορφώνεται ως εξής:

f(L2_size, L1i_size, L1d_size, L2_assoc, L1i_assoc, L1d_assoc, Cache_Line_Size) = (L2_assoc * L2_size/4096 + A * (L1i_assoc * L1i_size + L1d_assoc *  L1d_size)/256 ) * log(2 * Cache_Line_Size)

(Όπου log θεωρούμε λογάριθμο με βάση το 2)


* Simulation 1 score: 12.0625

* Simulation 2 score: 12.5

* Simulation 3 score: 20.0625
